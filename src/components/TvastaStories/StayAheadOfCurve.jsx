...
const StayAheadOfCurve = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row items-center gap-[60px] w-full max-w-[1280px] mx-auto px-4 md:px-8 lg:px-20">
      {/* Image Section */}
      <SlideInLeft delay={0.3} className="w-full lg:w-[508px] h-[300px] md:h-[350px] lg:h-[400px] flex-shrink-0">
        <div 
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-7 w-full">
            ...
            <div className="flex items-start gap-7">
              <AnimatedButton
                ...
                whileHover={(!email || !isChecked || isSubscribed) ? {} : { scale: 1.05 }}
                whileTap={(!email || !isChecked || isSubscribed) ? {} : { scale: 0.95 }}
                disabled={isSubscribed || !email || !isChecked}
              >
                <span className="font-outfit font-semibold text-[14px] md:text-[15px] lg:text-[16px] leading-[20px]">
                ...
                >
                  {isSubscribed ? (
                    ...
                    <svg className="w-3 h-3 text-[#0D192D]" fill="currentColor" viewBox="0 0 20 20">
